...
matrix sum_matrix_matrix(matrix M, matrix N) {
  if (M.row_dim != N.row_dim || M.col_dim != N.col_dim) {
    printf("Error las matrices son incompatibles\n");
    return M;
  }
...
vector *mult_matrix_vector(matrix M, vector v) {
  if (M.col_dim != v.dim) {
    printf("Error la matriz y el vector son incompatibles\n");
    return &v;
  }
...
int probarFuncionesVectores() {
  ...
  print_vector(v);
  printf("Dimensiones del vector: %d items\n", v.dim);
  destroy_vector(v);
  printf("\n");
...
int probarFuncionesMatrix() {
  ...
  print_matrix(M);
  printf("Dimensiones de la matriz: %d filas, %d columnas\n", M.row_dim, M.col_dim);
  destroy_matrix(M);
  printf("\n");
